<?php
	class Matrix
	{
		...
		private function check_matrix()
		{
			if (!($this->_preset))
				return false;
			if ($this->_preset == Self::SCALE && !($this->_scale))
				return false;
			if (($this->_preset == Self::RX || $this->_preset == Self::RY || $this->_preset == Self::RZ) && !($this->_angle))
				return false;
			if ($this->_preset == Self::TRANSLATION && !($this->_vtc))
				return false;
			if ($this->_preset == Self::PROJECTION && (!($this->_fov) || !($this->_ratio) || !($this->_near) || !($this->_far)))
				return false;
		}

		private function choose_matrix_type()
		{
			switch ($this->_preset)
			{
				case (Self::IDENTITY) :
